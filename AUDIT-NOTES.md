# Daily Bread 0.1.4 — audit fix map
...
| **DB-02** | P2 | `ensureCacheDir()` runs `mkdir -p ~/.cache/daily-bread` on bootstrap and before persist. `persistToDisk` logs `console.log` + sets `lastError` on failure (no empty catch). `printErrors` stays false; loadFailed also logs. |
...
| **DB-04** | P3 | Synced non-changelog surfaces to **0.1.4** (README layout, Panel comment, preview banner/title, `preview.svg`, DESIGN, PRE-SHIP, votd fallback). Changelog keeps 0.1.2 history. |
